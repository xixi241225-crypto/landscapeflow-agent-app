...
function Outputs({ blueprint, workflowStep, onAdvance, onExportJSON, onExportMarkdown, onNotice }) {
  ...
  return <div className="space-y-5">
    ...
    {workflowStep >= 2 && <Card title="步骤 2｜设计师确认方案文案" accent="var(--lf-gold)"><div className="grid gap-3 md:grid-cols-2"><label className="text-xs text-[var(--lf-muted)]">叙事重点<textarea className="form-input mt-1 min-h-[72px]" defaultValue={blueprint.coreNarrative?.title} /></label><label className="text-xs text-[var(--lf-muted)]">汇报口径与甲方表达<textarea className="form-input mt-1 min-h-[72px]" defaultValue="强调城市活力、全龄共享、低维护与可实施性。" /></label><label className="text-xs text-[var(--lf-muted)]">页面篇幅<input className="form-input mt-1" defaultValue="12 页，控制在 8–10 分钟汇报" /></label><label className="text-xs text-[var(--lf-muted)]">强调内容<input className="form-input mt-1" defaultValue="方案 B 的活力核心与融合的生态漫游体验" /></label></div>{workflowStep === 2 && <div className="mt-4 flex justify-end"><button onClick={onAdvance} className="btn-primary px-6 py-3 text-sm">确认文案并生成 PPT 结构</button></div>}</Card>}
